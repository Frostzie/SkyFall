 ...
 */
class StatusActionHandler(private val bottomBarView: BottomBarView?) {
    companion object {
    ...
    fun initialize() {
        EventBusOLD.register<EditorCursorChangedEvent> { event ->
            bottomBarView?.updateCursorPosition(event.line, event.column)
        }
    ...
    private fun handleStatusUpdate(event: StatusUpdateEvent) {
        when (event.type) {
            StatusType.CURSOR_POSITION -> {
                ...
                bottomBarView?.updateCursorPosition(line, column)
            }
