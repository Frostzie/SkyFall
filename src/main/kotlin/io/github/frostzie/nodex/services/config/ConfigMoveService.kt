 ...
 */
class ConfigMoveService(private val fileService: FileService) {
    ...
    fun moveConfig(source: Path, target: Path) {
        ...
        if (fileService.exists(target) && !fileService.isDirectoryEmpty(target)) {
        ...
        try {
            ...
            if (targetParent != null && !fileService.exists(targetParent)) {
                fileService.createDirectory(targetParent)
            }
