...
object FavoritePowerStone {
    ...
    fun onRenderSlot(context: DrawContext, slot: Slot) {
        ...
        if (slot == toggleSlotToRender && fakeItemToRender != null) {
            ...
            val mouseX = MinecraftClient.getInstance().mouse.x * currentScreen.width / MinecraftClient.getInstance().window.width
            val mouseY = MinecraftClient.getInstance().mouse.y * currentScreen.height / MinecraftClient.getInstance().window.height
            if (mouseX >= slot.x && mouseX < slot.x + 16 && mouseY >= slot.y && mouseY < slot.y + 16) {
                context.drawItemTooltip(client.textRenderer, fakeItemToRender, slot.x, slot.y)
            }
        }
